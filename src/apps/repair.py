...
def build_args():
    ...
    parser.add_argument(
        "--dont-clean-code",
        action="store_true",
        help="If provided, repaired code will *not* be cleaned before being inserted into the prompt.",
    )
    ...
    parser.add_argument(
        ...
        choices=["codellama_vllm_api", "chatgpt", "gpt4"],
    )
    parser.add_argument(
        '--num-threads',
        required=False,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='If provided, only repair the problems in this split.',
    )
    parser.add_argument(
        '--out-of',
        required=False,
        type=int,
        default=2,
        help='The number of splits to split the problems into.',
    )
    parser.add_argument(
        '--stop-if-at',
        required=False,
...
def repair_code(
    ...
    instruction = get_error_message(prob_path, errors, logger=logger)
...
def get_model(args):
    # Load the model
    if args.model == "codellama_vllm_api":
...
def main(args):
    ...
    if args.split is not None:
        # split ps into args.out_of equal groups, and only keep the args.split-th one
        ps = {k: v for i, (k, v) in enumerate(ps.items()) if i % args.out_of == args.split}
        logger.add_log_flush(
            "main",
            f"filtered out {length_after - len(ps)} problems that were not in split {args.split} (out of {args.out_of})",
            log_level=LogLevel.INFO,
        )
    ...
    def do_repair(problem, problem_json):
        ...
        local_logger.add_log(
            "main",
            f"trying to repair {len(jsons)} entries for prob_path {problem}",
            log_level=LogLevel.INFO,
        )
        ...
        if any(not j["binary"] for j in jsons):
            local_logger.add_log(
                "main",
                f"repairing {len(list(j for j in jsons if not j['binary']))} entries for prob_path {problem}",
                log_level=LogLevel.INFO,
            )
            ...
            for i, j in enumerate(jsons):
                if not j["binary"]:
                    if j.get("explanations", None) is None:
                        i_errors_code_expl.append((i, j["errors"], j["executed_completion"], None))
                    else:
                        for explanation in j["explanations"]:
                            i_errors_code_expl.append((i, j["errors"], j["executed_completion"], explanation))
            # bar = tqdm(total=len([j for j in jsons if not j["binary"]]), desc="Base programs executed")
            ...
            #         ): i
            #         for i, j in enumerate(jsons) if not j["binary"]
            #     }
            # for future in as_completed(future_to_idx):
            #     i = future_to_idx[future]
            #     errors, clean_code = future.result()
            #     passed = check_pass(errors)
            #     if passed:
            #         logger.add_log(f'main', f'Program for {problem} passed upon re-execution', log_level=LogLevel.WARNING)
            #         jsons[i]['binary'] = True
            #     else:
            #         i_errors_code.append((i, errors, clean_code))
            #     bar.update(n=1)
            #     gc.collect()
            to_exec = [] # list of (json, repairs) pairs to execute after getting all repairs
            # generate all the repairs
            bar = tqdm(total=len(i_errors_code_expl), desc="Repairs generated")
            with ThreadPoolExecutor() as p:
                future_to_idx = {
                    p.submit(
                        repair_code,
                    ...
                    for i, errors, clean_code, explanation in i_errors_code_expl
                }
            for future in as_completed(future_to_idx):
            ...
            with ThreadPoolExecutor() as p:
                future_to_data = {
                    p.submit(
                        exec_sample,
                        problem,
                        code): (i, completion, code, repairs[2], repairs[3], expl)
            ...
            for future in as_completed(future_to_data):
                ...
                if not explanation:
                    ...
                    explanation = completion.split(code)[0].rstrip(model.open_delim).rstrip()
                errors, clean_code = future.result()
                jsons[i]["repairs"].append(
                    {
                        "repaired_program": clean_code,
                        "explanation": explanation,
            ...
            local_logger.add_log(
                "main",
                f"no need to repair {len(jsons)} entries for prob_path {problem}",
                log_level=LogLevel.INFO,
            )
        ...
        local_logger.add_log_flush(
            "main",
            f"finished repairing {len(jsons)} entries for prob_path {problem}",
            log_level=LogLevel.INFO,
        )
        if args.output:
    ...
    if args.num_threads == 1:
        ...
        with ThreadPoolExecutor(max_workers=args.num_threads) as p:
            fs = [
                p.submit(do_repair, p, problem_json)
